...
def parse_dot(lines):
    ...
    for line in lines:
        ...
        if 'label=' in line and '->' not in line and 'node' not in line and 'graph' not in line:
        ...
        if e:
            ...
            edges.append((e.group('src'), e.group('tgt'), label.group(1) if label else ''))
...
def build_dot(label, nodes, roots, layers, edges):
    ...
    for r in roots:
        props = nodes[r]
        out.append(f'    "{r}" [label="{props["label"]}", fillcolor="{props["fillcolor"]}", rank=0];')
    out.append('')
    ...
    for nid, props in nodes.items():
        if nid in roots:
            continue
        out.append(f'    "{nid}" [label="{props["label"]}", fillcolor="{props["fillcolor"]}"];')
    out.append('')
...
def main():
    parser = argparse.ArgumentParser(description='Transform a DOT graph into layered clusters.')
